...
class BerryFieldEnv_MatInput(gym.Env):
    ...
    def render(self, mode="human", returnRGB=False):
        ...
        if l <= 0:
            line = rendering.Line(start=(-l, max(0, -b)),
                                  end=(-l, min(bounding_box[2], top)))
            line.set_color(0, 0, 255)
            self.viewer.add_onetime(line)
        if r >= 0:
            line = rendering.Line(start=(right, max(0, -b)),
                                  end=(right, min(bounding_box[2], top)))
            line.set_color(0, 0, 255)
            self.viewer.add_onetime(line)
        if b <= 0:
            line = rendering.Line(start=(max(0, -l), -b),
                                  end=(min(bounding_box[2], right), -b))
            line.set_color(0, 0, 255)
            self.viewer.add_onetime(line)
        if t >= 0:
            line = rendering.Line(start=(max(0, -l), top),
                                  end=(min(bounding_box[1], right), top))
            line.set_color(0, 0, 255)
            self.viewer.add_onetime(line)
