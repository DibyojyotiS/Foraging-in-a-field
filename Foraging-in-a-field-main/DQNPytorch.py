...
def select_action(state):
    ...
    if int(eps_threshold*1000) % 50 == 0:
        print(eps_threshold)
...
for i_episode in range(num_episodes):
    ...
    for t in count():
        ...
        if t % 500 == 0:
            print(f'episode = {i_episode}, count = {t}')
        next_state, reward, done, _ = env.step(action)
        # bestBerry = min(next_state, key=lambda l: 1e10 if l[3] == 0 else l[3])
        # reward += np.exp(-0.01*bestBerry[3])
