...
class BerryFieldEnv_MatInput(gym.Env):
    def __init__(self,
                 ...
                 # should divide respective dimention of field_size
                 reward_grid_size=(100, 100)
                 ):
        ...
        self.sectors = [((x-22.5) % 360, x+22.5)
                        for x in range(0, 360, 45)]  # for observatiof bucket type
        self.OBSHAPE = 35
        ...
        self.BERRY_COLLISION_TREE = collision_tree(
            bounding_boxes, boxIds, self.CIRCULAR_BERRIES, self.berry_radii)
        ...
        if reward_curiosity:
            assert all(f % z == 0 for f, z in zip(
                field_size, reward_grid_size))
    ...
    def step(self, action):
        ...
        if self.done and self.viewer is not None:
    ...
    def get_info(self):
        ...
        info = {
            ...
            'dist_from_edge': [
                # distance from left edge; w//2 if not in view
                w//2 - max(0, w//2 - x),
                # distance from right edge; w//2 if not in view
                w//2 - max(0, x+w//2 - W),
                # distance from the top edge; h//2 if not in view
                h//2 - max(0, y+h//2 - H),
                # distance from the bottom edge; h//2 if not in view
                h//2 - max(0, h//2 - y)
            ]
        }
    ...
    def ordered_observation(self):
        ...
        boxIds, boxes = self.get_Ids_and_boxes_in_view(
            (*self.state, *self.OBSERVATION_SPACE_SIZE))
        if len(boxIds) == 0:
            return observation
        ...
        data = np.column_stack(
            [np.ones_like(distances), directions, distances, boxes[:, -1]])
    ...
    def bucket_obseration(self):
        ...
        for i, sector in enumerate(self.sectors):
            if sector[0] < sector[1]:
                args = np.argwhere(
                    (angles >= sector[0]) & (angles <= sector[1]))
            else:
                args = np.argwhere(
                    (angles >= sector[0]) | (angles <= sector[1]))
    ...
    def curiosity_reward(self):
        ...
        if current_gridx >= self.size_visited_grid[0]:
            return 0
        if current_gridy >= self.size_visited_grid[1]:
            return 0
    ...
    def pick_collided_berries(self):
        ...
        # boxes are an array with rows as [x,y, size, size]
        sizes = boxes[:, 2]
    ...
    def get_Ids_and_boxes_in_view(self, bounding_box):
        boxIds, boxes = self.berry_collision_tree.boxes_within_bound(
    ...
    def argsort_clockwise(self, directions):
        tmepdirections = np.column_stack(
            [np.arange(directions.shape[0]), directions])

        def cmp(x, y): return self.isClockwise(x[1:], y[1:])
    ...
    def getTrueAngles(self, directions, referenceVector=[0, 1]):
        ...
        args0 = np.argwhere(np.bitwise_not(
            (curls > 0) | (curls == 0) & (dot == 1)))
    ...
    def render(self, mode="human", returnRGB=False):
        ...
        boxIds, boxes = self.get_Ids_and_boxes_in_view(
            observation_bounding_box)
        boxes[:, 0] -= self.state[0]-screenw/2
        boxes[:, 1] -= self.state[1]-screenh/2

        # adjust for my screen size
        scale = min(
            1, min(MAX_DISPLAY_SIZE[0]/screenw, MAX_DISPLAY_SIZE[1]/screenh))
        screenw, screenh = int(screenw*scale), int(screenh*scale)
        if self.viewer is None:
            self.viewer = renderingViewer(screenw, screenh)
        ...
        r = observation_bounding_box[0] + \
            observation_bounding_box[2]/2 - self.FIELD_SIZE[0]
        b = observation_bounding_box[1] - observation_bounding_box[3]/2
        t = observation_bounding_box[1] + \
            observation_bounding_box[3]/2 - self.FIELD_SIZE[1]
        top = observation_bounding_box[3] - t
        right = observation_bounding_box[2] - r
        if l <= 0:
            line = rendering.Line(
                start=(-l, max(0, -b)), end=(-l, min(observation_bounding_box[2], top)))
            line.set_color(0, 0, 255)
            self.viewer.add_onetime(line)
        if r >= 0:
            line = rendering.Line(
                start=(right, max(0, -b)), end=(right, min(observation_bounding_box[2], top)))
            line.set_color(0, 0, 255)
            self.viewer.add_onetime(line)
        if b <= 0:
            line = rendering.Line(
                start=(max(0, -l), -b), end=(min(observation_bounding_box[2], right), -b))
            line.set_color(0, 0, 255)
            self.viewer.add_onetime(line)
        if t >= 0:
            line = rendering.Line(
                start=(max(0, -l), top), end=(min(observation_bounding_box[1], right), top))
            line.set_color(0, 0, 255)
            self.viewer.add_onetime(line)
